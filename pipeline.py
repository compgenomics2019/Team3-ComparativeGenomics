...
=======
#!/usr/bin/env python
>>>>>>> d1a1ceb8759595b8638b5e8bbb00bfba3d98d130
import argparse
import os
...
<<<<<<< HEAD
def check_files(file_handle):
	#sys.exit if empty or not in correct format
	
	#print("okay")
	tmp_lst=file_handle.readlines()
	if len(tmp_lst)==0:
		sys.exit()
	#check is the file is in correct format
	else:
		for line in range(0,len(tmp_lst)):
			if tmp_lst[line].startswith(">"):
				for l in range(line,len(tmp_lst)):
					if tmp_lst[l].startswith(">"):
						break
					else:
						tmp_lst[l]=tmp_lst[l].strip("\n").strip(" ")
						if not re.search("^[ATGCNatgcn]+$",tmp_lst[l]):
							sys.exit() 
...
<<<<<<< HEAD
	if not args.ANI or not args.MLST:
=======
